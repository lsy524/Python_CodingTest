pos = [0] * 8                                           # 각 열에 배치한 퀸의 위치    
flag_a = [False] * 8                                    # 각 행에 퀸을 배치했는지 체크 
flag_b = [False] * 15                                   # 대각선 방향(/)으로 퀸을 배치했는지 체크
flag_c = [False] * 15                                   # 대각선 방향(\)으로 퀸을 배치했는지 체크

# def put() :
#     for i in range(8):
#         print(f"{pos[i]:2}", end="")
#     print()

# 각 열에 배치한 퀸의 위치를 출력
def put():
    for i in range(8):
        for j in range(8):
            print("■" if pos[j] == i else "□", end="")  # 퀸의 배치를 □와 ■로 출력
        print()
    print()

# i열의 알맞은 위치에 퀸을 배치 
def set(i):
    for j in range(8):
        if( not flag_a[i] and                           # j행에 퀸이 배치되지 않았다면
            not flag_b[i+j] and                         # 대각선 방향(/)으로 퀸이 배치되지 않았다면
            not flag_c[i-j+7]):                         # 대각선 방향(\)으로 퀸이 배치되지 않았다면

            pos[i] = j                                  # 퀸을 j행에 배치
            if i == 7:                                  # 모든 열에 퀸을 배치 완료
                put()
            else :
                flag_a[i] = flag_b[i+j] = flag_c[i-j+7] = True
                set(i + 1)                              # 다음열에 퀸을 배치 
                flag_a[i] = flag_b[i+j] = flag_c[i-j+7] = False

set(0)                                                  # 0열에 퀸을 배치